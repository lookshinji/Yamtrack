...
class CalendarMainTests(CalendarFixturesMixin, TestCase):
    ...
    @patch("events.calendar.main.get_items_to_process")
    def test_fetch_releases_empty_items_list_returns_without_selector_lookup(
        self,
        mock_get_items_to_process,
    ):
        """An explicit empty list should not trigger selector discovery."""
        result = fetch_releases(items_to_process=[])

        self.assertEqual(result, "No items to process")
        mock_get_items_to_process.assert_not_called()

    @patch("events.calendar.main.process_podcast")
    def test_fetch_releases_routes_podcast_items(self, mock_process_podcast):
        """Podcast items should be routed through the dedicated processor."""
        ...
        def add_podcast_event(item, events_bulk):
            events_bulk.append(
                Event(
                    item=item,
                    content_number=1,
        ...
        mock_process_podcast.side_effect = add_podcast_event

        result = fetch_releases(items_to_process=[podcast_item])

        mock_process_podcast.assert_called_once()
        self.assertEqual(mock_process_podcast.call_args[0][0], podcast_item)
        self.assertTrue(Event.objects.filter(item=podcast_item).exists())
        self.assertIn("Podcast Episode", result)
